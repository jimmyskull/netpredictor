...
uNetwalk <- function(ig, normalise=c("row","column","laplacian","none"), dataSeed=NULL, restart=0.8, parallel=TRUE, multicores=NULL, verbose=T)
{
    ...
    A <- adjM != 0
    if(normalise == "row"){
        D <- Matrix::.sparseDiagonal(n=vcount(ig), x=(Matrix::rowSums(A) + 1e07)^(-1))
        nadjM <- adjM %*% D
    }else if(normalise == "column"){
        D <- Matrix::.sparseDiagonal(n=vcount(ig), x=(Matrix::colSums(A) + 1e07)^(-1))
        nadjM <- D %*% adjM
    }else if(normalise == "laplacian"){
        D <- Matrix::.sparseDiagonal(n=vcount(ig), x=(Matrix::colSums(A) + 1e07)^(-0.5))
        nadjM <- D %*% adjM %*% D
    }else{
        nadjM <- adjM
    }
    ...
    colNorm<- function(m){
        ...
        col_sum_matrix <- Matrix(rep(col_sum, nrow(m)), ncol=ncol(m), nrow=nrow(m), byrow=T, sparse=T)
    ...
    if(is.null(dataSeed)){
        ...
        P0matrix <- .sparseDiagonal(vcount(ig))
        ...
        if(is.matrix(dataSeed) | is.data.frame(dataSeed)){
            data <- matrix(dataSeed)
        }else if(is.vector(dataSeed)){
            data <- matrix(dataSeed, ncol=1)
        }
        ...
        if (class(ig) == "igraph"){
            ...
            P0matrix[ind[!is.na(ind)],] <- Matrix(data[!is.na(ind),])
    ...
    if(restart==1){
        ...
        if(parallel==TRUE){
            ...
            if(flag_parallel){
                j <- 1
                cat("For each, j=", j, "to", ncol(P0matrix), "\n")
